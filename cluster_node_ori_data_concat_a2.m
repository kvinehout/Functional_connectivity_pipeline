function [ ts ] = cluster_node_ori_data_concat_a2(PATH_NAME,subject_name,ica_file,good_compi,file_length,lengthcomp,ROIGM,task_name)
...
for compi=good_compi %for each good_compi values
   ...
   for subji=1:length_subject_name  
       ...
       if ROIGM==0
            timecourse_data=sprintf('fslmeants -i %s -o %d_timecourse_comp%d_subji_%d_%d_%snewtwo.txt -m %s/mask_comp_whole%d.nii.gz',subject_name{subji},count,compi,subji,lengthcomp,task_name,ica_file,compi);
       elseif ROIGM==1
            timecourse_data=sprintf('fslmeants -i %s -o %d_timecourse_comp%d_subji_%d_%d_%snewtwo.txt -m %s/mask_comp_whole_GM_%d.nii.gz',subject_name{subji},count,compi,subji,lengthcomp,task_name,ica_file,compi);
       else
           warning('ROIGM not defined correctly')
       end
       %unix(timecourse_data);%this line takes up a lot of time --> anyway to speed up? 
       textname=sprintf('%d_timecourse_comp%d_subji_%d_%d_%snewtwo.txt',count,compi,subji,lengthcomp,task_name);

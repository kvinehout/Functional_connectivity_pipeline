...
%{
  ...
  elseif current_subj(1)=='S'%for only stroke subjects
                                     ...
                                     %replicate the warped image into a 4D warped image for
                                     %FMRI FOR LINDA 
                                    IMdiffmath=sprintf('ImageMath 3 four_d_deformed_fmri_LINDA_num%d.nii.gz ReplicateDisplacement %s_CollapsedWarpLINDA.nii.gz %d %d 0',subject,anat,hislice,tr);
                                    unix(IMdiffmath)
                                    %replicates the template image into 4D
                                    IMrepmath=sprintf('ImageMath 3 four_d_temp_fmri_LINDA_num%d.nii.gz ReplicateImage %s/pennTemplate/template.nii.gz %d %d 0',subject,linda_path,hislice,tr);
                                    unix(IMrepmath)
                                    %apply the warped data to the 4D data
                                    applyants=sprintf('antsApplyTransforms -d 4 -o %s_standardLINDA.nii.gz -t four_d_deformed_fmri_LINDA_num%d.nii.gz -r four_d_temp_fmri_LINDA_num%d.nii.gz -i %s.nii.gz',fmri,subject,subject,fmri);
                                    unix(applyants)
                                     %FMRI FOR REFERANCE
                                    IMdiffmath=sprintf('ImageMath 3 four_d_deformed_fmri_num%d.nii.gz ReplicateDisplacement %s_CollapsedWarp.nii.gz %d %d 0',subject,anat,hislice,tr);
                                    unix(IMdiffmath)
                                    ...
                                    unix(IMrepmath)                     
                                    %apply the warped data to the 4D data
                                    applyants=sprintf('antsApplyTransforms -d 4 -o %s_standard.nii.gz -t four_d_deformed_fmri_num%d.nii.gz -r four_d_temp_fmri_num%d.nii.gz -i %s_standardLINDA.nii.gz',fmri,subject,subject,fmri);
                                    unix(applyants)
                                    %clear variables
                                    clear tr
                                    clear hislice  

                                    %s_or_brain.nii.gz -m %s/linda/Subject_in_MNI.nii.gz -o %s/linda/ANA_to_MNI_to_ref -x %slinda/Lesion_in_MNI.nii.gz -j 1',ref,anat_short,anat_short,anat_short);
                                                                        %this transforms anat to the reg image 
                                    warp=sprintf('WarpImageMultiTransform 3 %s/linda/Subject_in_MNI.nii.gz %s_warpimtransform.nii.gz -R %s/linda/ANA_to_MNI_to_ref.nii.gz %s/linda/ANA_to_MNI_to_ref1Warp.nii.gz %s/linda/ANA_to_MNI_to_ref0GenericAffine.mat', anat,anat,anat_short,anat_short,anat_short);
                                    unix(warp)
                                    %collapse the warp file
                                    collapsewarp=sprintf('antsApplyTransforms -d 3 -o [%s_CollapsedWarp.nii.gz,1] -r %s_or_brain.nii.gz -t %s/linda/ANA_to_MNI_to_ref1Warp.nii.gz -t %s/linda/ANA_to_MNI_to_ref0GenericAffine.mat',anat,ref,anat_short,anat_short);
                                    unix(collapsewarp)
%}

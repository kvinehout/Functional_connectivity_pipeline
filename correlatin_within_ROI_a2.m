function [ subject_mask_avg_new,subject_mask_avg,stdev_sub] = correlatin_within_ROI_a2(path_name,ica_file,subject_name,good_compi,fmri_length,ppi_subject,TR,timmingfile,ROIGM,task_name)
...
for compi=good_compi 
    ...
    for subji=1:length_subject_name  
                ...
                % filename=sprintf('%s/%s_ICA_AROMA/%s.feat/sub1.results/errts.sub1.nii',subject_name{subji});
                 subject_name_one=sprintf('%s',subject_name{subji});%,subject_name{subji},filename);
                    ...
                    textfilename=sprintf('ROI%d_subj_%d_%s.txt',compi,subji,task_name);
                  ...
                  %  unix(sprintf('3dmaskdump -noijk -xyz -mask %s %s > %s',mask_name,subject_name_one,textfilename));
                    ...
                    if ppi==0
                        netmats1Z=nets_netmats(ts_subMask,0,'corr'); 
    ...
    clear  mask_compi 
    %save compi data
    %this is current not individual file subject_mask_avg
    %note saving this file takes a long time..... remove? 
    save_name=sprintf('within_mean_corr_comp_%dZ_%s',compi,task_name);
    save(save_name,'subject_mask_avg') 
end


%% APPLY FISHER Z TRANSFROM TO DATA 
%{
%DONT NEED BECUASE ALREADY DONE ABOVE
ts.Nsubjects=length_subject_name;
ts.NtimepointsPerSubjec=size(subject_mask,4);
MethodType=1;
 % quick crappy estimate of median AR(1) coefficient
  ...
  subject_mask_avg_FZ_Ztrasn=0.5*log((1+subject_mask_avg)./(1-subject_mask_avg))*RtoZcorrection;
%}
